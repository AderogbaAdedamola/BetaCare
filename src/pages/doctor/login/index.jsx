...
import { DoctorLoginPanel } from "../../../components/auth/AuthSidePanel";
...
export default function DoctorLogin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = await api.post("/auth/doctor/login", form);
  ...
  return (
    <AuthLayout sidePanel={<DoctorLoginPanel />} panelVisible={true}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <p className="text-sm text-muted-foreground mb-8">Access your patients and clinical dashboard</p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="flex justify-end mt-1.5">
              <Link to="/password-reset?role=doctor" className="text-xs text-primary font-medium hover:underline">Forgot password?</Link>
            </div>
          </div>
        ...
        <p className="text-sm text-muted-foreground mt-6 text-center">
          Not registered?{" "}
          <Link to="/doctor/register" className="text-primary font-medium hover:underline">Apply as a doctor</Link>
        </p>
